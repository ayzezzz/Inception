...
For security reasons, sensitive credentials are not stored in the environment or Dockerfiles:
- **Location**: All passwords and keys are stored as plain text files within the `secrets/` directory at the project root.
- **Management**: To change or locate credentials, refer to the files inside the `secrets/` folder.
